...
use scheduler::store::backing::Backing;
use scheduler::store::{Error, Store};
...
async fn upsert<B>(data: web::Data<Data<B>>, job: web::Json<ScheduledJob>) -> HttpResponse
where
...
impl Server {
    ...
    ///
    #[inline]
    pub async fn run<B>(job_store: Store<B>, address: &str) -> anyhow::Result<()>
    where
        B: Backing + Send + Sync + 'static,
    {
